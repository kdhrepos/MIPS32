...
void fetch(MIPS32Simulator * sim)
{
    ...
    {
        printf("taken\n");
        inst = sim->memory[sim->ex_mem_reg.br_trgt];
    }
...
void execute(MIPS32Simulator * sim)
{
    ...
    if(ALUOp == 0x2) // b'10
    {
        if(ALU_ctrl_input == 0x3) /* add, b'0010 */
    ...
    sim->ex_mem_reg.br_trgt = pc + (imm_val << 2);
...
void memory(MIPS32Simulator * sim)
{
    ...
    int br_trgt = sim->ex_mem_reg.br_trgt;
